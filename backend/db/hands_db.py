...
class DatabaseHands:
    ...
    async def get_all_user_games(self, _id) -> list[dict[Any, Any]]:
        async with self.db.get_session() as session:
            result = await session.execute(
                text("""
    ...
    async def get_game_info(self, code) -> Any:
        async with self.db.get_session() as session:
            result = await session.execute(
    ...
    async def get_room_info(self, code) -> list[dict[Any, Any]]:
        print(code)
        async with self.db.get_session() as session:
            result = await session.execute(
                text(
    ...
    async def create_game(self, admin_id, title, join_code, scheduled_at) -> None:
        async with self.db.get_session() as session:
    ...
    async def insert_or_update_team(self, game_id, team_id, team_name) -> bool:
        async with self.db.get_session() as session:
            result = await session.execute(
                text(
    ...
    async def get_team_name(self, team_id) -> Response:
        async with self.db.get_session() as session:
            result = await session.execute(
    ...
    async def update_score_team(self, game_id, nickname, add_score) -> None:
    ...
    async def update_game_any_param(self, join_code, column, value) -> None:
